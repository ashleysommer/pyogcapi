...
class Plugin(object, metaclass=PluginMeta):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if cls in PluginGlobals.singleton_services(env):       #pragma:nocover
            ...
            if self is True:
                ...
                self._p_name = self.__class__.__name__
                self.activate()
            self._enable = True
        ...
        #
        self.id = PluginGlobals.next_id()
        self._p_name = "Plugin."+str(self.id)
        self._enable = True
    ...
    def __repr__(self):
        ...
        if self.__class__.__name__ == self._p_name:
            return '<Plugin %s>' % (self.__class__.__name__)
        else:
            return '<Plugin %s %r>' % (self.__class__.__name__, self._p_name)
...
def CreatePluginFactory(_interface):
    ...
    class PluginFactoryFunctor(object):
        def __call__(self, _name=None, args=None, **kwds):
            if _name is None:
                return self
            if args is None:
                args = tuple()
...
def PluginFactory(classname, args=None, env=None, **kwds):
    ...
    obj = cls(*args, **kwds)
    if 'name' in kwds:
        obj._p_name = kwds['name']
    if __debug__:
        if obj is None:
            PluginGlobals.env().log.debug("Failed to create plugin %s" % (classname))
        else:
            PluginGlobals.env().log.debug("Creating plugin %s with name %s" % (classname, obj._p_name))
    return obj
